...
export async function listStaffAction(): Promise<Staff[]> {
  try {
    ...
    return (data ?? []).map(s => ({
      ...
      discount: Number(s.discount) || 0, note: s.note ?? null,
    }))
  } catch { return [] }
}
...
export async function saveStaffAction(id: string, fields: Partial<Omit<Staff, 'id'>>): Promise<ActionResult> {
  try {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return { success: false, error: 'No autenticado' }
    const keys = ['name', 'phone', 'emergency_phone', 'insurance_no', 'branch', 'salary', 'days_worked', 'absences', 'discount', 'note'] as const
