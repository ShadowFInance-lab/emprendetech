...
export default function SocialAuthButtons() {
  ...
  async function signInWithGoogle() {
    ...
    if (error) {
      ...
      toast.error(
        notEnabled
          ? 'Activa Google en Supabase → Authentication → Providers para usar este botón.'
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-2 h-11 rounded-xl border border-gray-200 hover:bg-gray-50 transition-colors font-medium text-sm text-gray-700 disabled:opacity-60"
      >
        {loading ? (
